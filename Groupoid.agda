...
hom-sy G {a} {b} = sym {A = Hom G a b}

hom-tr : (G : Groupoid i) {a b : _} {p q r : Hom G a b} → p ≡ q → q ≡ r → p ≡ r
hom-tr G {a} {b} = trans {A = Hom G a b}
...
module _ (Γ : Con i) {a b c d}
  ...
  HomEq-eq12 = trans {A = Prop i} (cong (λ z → HomEq Γ z k2 p q) eq1)
...
module _ (Γ : Con i) {a b c d}
  ...
  HomEq-eq34 = trans {A = Prop i} (cong (λ z → HomEq Γ _ _ z q) eq1)
...
module _ (G : Groupoid i) {x y : ∣ G ∣} {p : Hom G x y} where
  ...
  inv-unique {q = q} eq =
    sym {A = Hom G _ _} (trans {A = Hom G _ _} (trans {A = Hom G _ _}
      (trans {A = Hom G _ _} (sym {A = Hom G _ _} (id2 G)) aux'') aux')
      (trans {A = Hom G _ _} aux (id1 G)))
    where aux = cong (T G (S G p)) eq
          aux' = sym {A = Hom G _ _} (assoc G {p = S G p} {p} {q})
          aux'' = cong (λ z → T G z q) (sym {A = Hom G _ _} (inv2 G))
...
module _ {G : Groupoid i} {H : Groupoid j} {K : Groupoid k} where
  ...
  comp-fun h1 h2 = record
    ...
    ; f-R = λ {x} -> trans {A = Hom K _ _} (cong (f1 h2) (f-R h1 {x})) (f-R h2)
    ; f-T = λ p q → trans {A = Hom K _ _} (cong (f1 h2) (f-T h1 p q)) (f-T h2 _ _)
    }
