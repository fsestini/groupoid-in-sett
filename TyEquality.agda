  ...
  _~*' a₀ a₁ = record
    ...
    ; tm1 = λ { {δ , p} {δ' , p'} (q , r) →
        let goal : Iso (IxHom A p (tm0 a₀ δ) (tm0 a₁ δ)) (IxHom A p' (tm0 a₀ δ') (tm0 a₁ δ'))
            goal = record
                   { iso1 = λ f → transp-IxHom A (unprf r) (IxT A (IxS A (tm1 a₀ q)) (IxT A f (tm1 a₁ q)))
                   ; iso2 = λ f → transp-IxHom A (hom-tr Γ (cong (λ z → T Γ z (T Γ p' _))
                                    (hom-sy Γ (SS-id Γ))) (HomEq-S Γ (unprf r)))
...
module _ {Γ : Con i} (A : Ty j Γ) (ρ₀ ρ₁ : Ω ⟶ Γ) (γ₀ γ₁ : Δ ⟶ Ω)
  ...
  _~~* = record
    { tm0 = λ δ →
          IxHomEq A (unprf (tm0 r δ)) (tm1 a₀ (tm0 p δ)) (tm1 a₁ (tm0 p δ))
                    (tm0 q₀ δ) (tm0 q₁ δ)
    ; tm1 = λ {γ} {γ'} h →
...
module _ {Γ : Con i} {Δ : Con k} (A : Ty j Γ) (γ₀ γ₁ : Δ ⟶ Γ)
  ...
  coh~ = record
    ...
    ; tm1 = λ {δ} {δ'} q →
        let goal : IxHomEq A (unprf (tm1 p q)) (tm1 a q) (tm1 (coe~ A γ₀ γ₁ p a) q)
                         (R (∣ A ∣* (f0 γ₁ δ)) (f0 (subst* A (tm0 p δ)) (tm0 a δ)))
                         (R (∣ A ∣* (f0 γ₁ δ')) (f0 (subst* A (tm0 p δ')) (tm0 a δ')))
            goal = IxHomEq-R A (subst*-eq A (unprf (tm1 p q)) (tm1 a q))
        in prf goal
    ; tm-refl = ttp
--    ; tm-trans = ttp
    }
